...
#define ADDR_STAGE_START_TIME    100 // Bytes 100-107 (8 bytes - time_t)
#define ADDR_STAGE_STARTED_FLAG  108 // Byte 108 (1 byte - bool)
#define ADDR_CONFIG_SAVED        109 // Byte 109 (1 byte - flag de validação)
#define ADDR_TARGET_REACHED_FLAG 110 // Byte 110 (1 byte - bool) ✅ NOVO
// Reservado: 111-127 para expansão

// -----------------------------------------------
// SEÇÃO 3: CONFIGURAÇÕES GERAIS (128-191) - 64 bytes
// -----------------------------------------------
#define ADDR_GENERAL_CONFIG_START 128
#define ADDR_LAST_VALID_EPOCH     128 // Bytes 128-135 (8 bytes - time_t)
#define ADDR_LAST_VALID_MILLIS    136 // Bytes 136-139 (4 bytes - unsigned long)
...
inline void printEEPROMLayout() {
    #if DEBUG_EEPROM
    Serial.println(F("\n╔═══════════════════════════════════════════╗"));
    Serial.println(F("║      LAYOUT DA EEPROM CORRIGIDO (V4)      ║"));
    Serial.println(F("╠══════════════════════════════════════════╣"));
    ...
    Serial.printf( "║ Timestamp:      %3d-%3d (8b)            ║\n", 
                   ADDR_STAGE_START_TIME, ADDR_STAGE_START_TIME + 7);
    Serial.printf( "║ Flags:          %3d, %3d, %3d            ║\n", 
                   ADDR_STAGE_STARTED_FLAG, ADDR_CONFIG_SAVED, ADDR_TARGET_REACHED_FLAG);
    Serial.println(F("╚═══════════════════════════════════════════╝\n"));
    #endif
}
...
inline void debugEEPROMContents() {
    ...
    EEPROM.get(ADDR_TARGET_REACHED_FLAG, targetReached);
    ...
    if (configSaved) {
        ...
        Serial.printf( "║ Tempo:         %-26lld ║\n", (long long)stageStartTime);
    }
    Serial.println(F("╚═══════════════════════════════════════════╝\n"));
    #endif
}
